...
class ModelNetDataLoader(Dataset):
    ...
    def __init__(self,
        ...
        for i,sn in enumerate(shape_names):
            if sn not in self.classes:
                shape_names[i] = sn.split('_')[0]
        ...
        self.dl = dl  # delimiter for loading txt files
    ...
    def _get_item(self, index):
        if index in self.cache:
            ...
            #                                [0, 0, 1]])
            #     point_set[:, 0:3] = np.dot(point_set[:, 0:3], rotation_matrix)
                
               
            #     # 4. 添加高斯噪声
            #     noise = np.random.normal(0, 0.01, size=point_set[:, 0:3].shape)
            #     point_set[:, 0:3] += noise
